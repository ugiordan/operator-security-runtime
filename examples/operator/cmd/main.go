...
import (
	...
	"github.com/opendatahub-io/operator-security-runtime/examples/operator/internal/controller"
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "1f74e49c.example.com",
	...
	operatorNS := getEnvOrDefault("OPERATOR_NAMESPACE", "k8s-serviceaccount-hijacking-protection-system")
	...
	if err = (&controller.ExampleResourceReconciler{
		...
		setupLog.Error(err, "unable to create controller", "controller", "ExampleResource")
		os.Exit(1)
	}
	// nolint:goconst
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		protectedIdentities := []saprotection.ProtectedIdentity{
			{
				Namespace:          getEnvOrDefault("OPERATOR_NAMESPACE", saprotection.ExampleOperatorNamespace),
				ServiceAccountName: getEnvOrDefault("OPERATOR_SA_NAME", saprotection.ExampleOperatorServiceAccountName),
			},
		}
		...
		setupLog.Info("webhook registered", "protectedIdentities", protectedIdentities)
	} else {
		setupLog.Info("WARNING: webhooks are disabled via ENABLE_WEBHOOKS=false")
	}
